...
impl Display for LogicError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "LOGIC ERROR: ");
        match self {
            LogicError::InvalidCategoryRelation { parent, child } => write!(f, "Category Edge {parent} to {child} would create loop!"),
...
impl Display for DBError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "DB ERROR: ");
        match self {
            DBError::RestrictViolation => write!(f, "Unable to delete restricted DB Row"),
            DBError::External(error) => write!(f, "{error}"),
        }
    }
}
